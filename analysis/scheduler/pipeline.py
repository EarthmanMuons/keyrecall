...
def challenge_bypass(
    ...
    """Named exceptions (§6). new_material: no MaterialMemoryState entry
    yet. recovery: the prior attempt in this session failed. override:
    caller-supplied, for diagnostic-probe/explicit-learner-request
    scenarios that aren't derivable from state alone."""
...
def retention(prediction: Prediction) -> float:
    """R(e), §7.2/03-v1-math.md §23's simplest form: 1 - M, ignoring
    uncertainty for this pass."""
...
def information(
    ...
    (retrieval_demand/retrieval_observed), applied in the same direction,
    without needing an Outcome (unavailable pre-attempt). Read-only:
    never creates a MaterialMemoryState/MaterialExecutionState entry, so
    computing this is never itself evidence."""
    ...
    memory_weight = retrieval_demand if retrieval_observed else 0.0
    memory_term = memory_uncertainty * memory_weight
