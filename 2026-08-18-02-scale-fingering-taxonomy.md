...
Canonical runtime fingering must be validated against
octave-continuation behavior. A terminal one-octave fingering alone is
insufficient evidence when initial, internal, or terminal tonic behavior
may differ. Compact one-octave strings in this document are therefore
display summaries, not substitutes for multi-octave normalization.
...
The final V1 selection is supported by more than contemporary fingering
charts. A 2026 peer-reviewed *Music Theory Online* analysis of
historical keyboard scale pedagogy reports that, in Hanon's scale
regimen, RH harmonic- and melodic-minor fingerings coincide in every key
except C-sharp and F-sharp minor. A university piano curriculum
independently separates C-sharp and F-sharp melodic minor from their
natural/harmonic minor fingering groups.

This historical evidence concerns classical melodic minor. KeyRecall
uses it only for the **ascending fingering**, whose pitch collection is
the same `1 2 b3 4 5 6 7` collection used by fixed-form/jazz melodic
minor. KeyRecall then retains that same collection and reverses its
fingering on descent; it does not adopt the classical natural-minor
descending pitch collection.

Together with the previously recorded all-key fingering corroboration,
this is sufficient to freeze `23123412` as the V1 RH canonical fingering
...
The completed multi-octave audit strongly suggests that the V1 canonical
corpus is much more structurally compact than its 96 hand-specific
records imply.

Most seven-note scale fingerings appear to be phase and boundary
variants of the traditional alternating three- and four-finger grouping:
...
This is an input hypothesis for the separate `MotorFamily` derivation,
not a finalized taxonomy. The canonical fingering records remain the
source of truth. Future alternative fingerings may map to an existing
motor family, a different phase or boundary realization, or a new motor
family without changing the canonical V1 dataset.
...
14. `MotorFamily` classifications should now be derived from the
    ...
    labels alone.
15. The canonical dataset must remain independent of the derived motor
    taxonomy so future alternative fingerings can be added without
...
Brown and Lee's 2026 *Music Theory Online* study provides historical
support for the C-sharp/F-sharp RH melodic-minor exception in Hanon's
scale pedagogy.
...
PDM Piano groups scales by fingering pattern and independently treats
C-sharp and F-sharp melodic minor separately from their corresponding
natural/harmonic minor patterns.

<https://www.pdmpiano.org/epdm_p240_tech.html>

### Multi-octave F-sharp natural-minor corroboration

Multi-octave pedagogical material supports RH `34123123` and LH
`43213214` for F-sharp natural minor.

<https://vitapiano.com/piano-scales/f-sharp-minor-scale/>

## 23. Document Status

The V1 canonical scale-fingering research and multi-octave normalization
pass are complete.

This document records all 12 major, 12 natural-minor, 12 harmonic-minor,
and 12 fixed-form melodic-minor scales: **48 scale definitions and 96
hand-specific canonical records**, together with the provenance needed
for meaningful exceptional choices.

The canonical fingering dataset is now considered the source of truth
for V1. Future alternative fingerings may be added as separate
provenance-backed records without replacing these canonical choices.

Formal `MotorFamily` derivation, phase analysis, technical-event
enumeration, and Q-matrix implications should proceed in a separate
document using this completed corpus as input.
