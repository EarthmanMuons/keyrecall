...
`consolidated_log_half_life_variance` has a narrower contract than the other
uncertainty fields: it is the posterior variance of
`log_consolidated_half_life`. Current-durability and cold-start uncertainty
remain heuristic confidence quantities and are not interchangeable with it.
