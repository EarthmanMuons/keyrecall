...
fingerings\
**Input:** `2026-08-18-08-scale-fingering-taxonomy.md`\
**Machine-readable mapping:** `2026-08-18-scale-motor-realizations.yaml`
/ `.csv`
...
The canonical fingering taxonomy remains the source of truth for the
actual fingers expected for each scale and hand. This document does not
generate or replace canonical fingering. It derives a structural layer
that can support transfer, diagnosis, and learner-state inference.
...
Future alternative fingerings may map into the same structural model or
expand it.
...
dataset, **all 96 canonical V1 hand-specific records map to one motor
family**.
...
with hand orientation, cycle phase, boundary behavior, scale topology,
and keyboard geometry supplying the important contextual distinctions.
...
These are two hand-specific orientations of one higher-order scale motor
family, not two unrelated skills.

## 4. Rotation Equivalence

Every tonic-relative continuation cycle in the verified V1 corpus is a
rotation of the appropriate hand orientation.
...
The C-sharp/F-sharp fixed-form melodic-minor RH pattern that initially
appeared to require a second family:
...
Thus the documented melodic-minor exception is a canonical fingering
exception at the scale level, but **not a separate motor family** under
...
2.  The current LH corpus uses six of seven possible phases; absence of
    LH phase 2 is a property of the selected canonical V1 fingerings,
    not a statement that the phase is mechanically invalid.

A future alternative fingering could introduce an unused phase without
requiring a new motor family.
...
`MotorRealization` is derived from canonical fingering. It must never
become the authoritative source from which fingering is silently
reconstructed.

## 7. Boundary Behavior

The machine-readable corpus distinguishes entry and terminal adaptations
...
The three/four grouping terminology remains pedagogically useful, but it
should not be encoded as mutually exclusive motor-family IDs.
...
are rotations of one cyclic structure, the difference is one of **phase
and grouping perception**, not rotation-invariant motor-family identity.

The UI or pedagogy may still describe a passage as "group four, then
three." Internally, KeyRecall should represent the exact phase and
technical crossings rather than create a second latent family.
...
-   potentially fail to fit `DIATONIC_3_4_CYCLE`, in which case the
    taxonomy can add a new family based on evidence.
...
-   **47 distinct crossing signatures** when restricted to thumb/finger
    crossings and their geometry.

Those counts are evidence that the event representation should be
**attribute-based**, not implemented as 170 or 47 independent enum
values or learner skills.
...
These counts are descriptive of the generated audit corpus, not
pedagogical weights.
...
The four 3-semitone cases arise from harmonic-minor augmented-second
topology.

This supports the earlier conclusion that a single
`BLACK_KEY_NAVIGATION` learner variable would be too coarse.
...
For example, the same crossing finger can function as the scale's
ordinary within-octave thumb passage in one phase and as an
...
The enum should be driven by actual diagnostic needs rather than by
every structural distinction available in the generator.
...
different `structure` fields rather than being shoehorned into crossing
events.

## 15. What Should Become a Learner Component?

The mechanical analysis argues strongly **against** one latent learner
component for every event signature.
...
conditioning the evidence** rather than dozens of independent learner
skills.

If longitudinal data later shows stable geometry-specific weaknesses,
the learner model can promote selected contexts into explicit latent
components.

## 16. Phase in the Learner Model

The mechanical result confirms that phase is structurally important but
does not justify seven separate phase skills.
...
-   aggregate primary learning evidence at the shared motor/crossing
    components; and
...
different phase changes which crossing occurs in which structural
location, and the event stream supplies that diagnostic context.

Likewise C-sharp fixed melodic minor RH does not require a separate
`FOUR_THEN_THREE` learner component. Its distinctive canonical fingering
is represented by its phase and event sequence within
`DIATONIC_3_4_CYCLE`.
...
A pianist demonstrating fluent execution of one scale supplies evidence
for:
...
This gives KeyRecall a principled middle ground between treating scales
as isolated skills and treating them as interchangeable.
...
Contains every adjacent transition generated for two-octave ascending
and descending audit exercises across all 96 records.

These are analysis artifacts, not yet the proposed production data
format.
...
-   one-octave display strings are insufficient as authoritative
    continuation data;
-   detailed technical events are best represented compositionally
    through attributes; and
-   alternative fingering support can extend the model without changing
    the V1 canonical dataset.
...
-   whether keyboard-geometry effects can be summarized with a small set
    of useful contextual features;
-   how strongly evidence transfers across hand, direction, phase, and
    geometry; and
-   whether future modes, alternative fingerings, or non-diatonic scales
    introduce additional `MotorFamily` values.

## 22. Next Step

The structural fingering analysis is now sufficiently mature to return
to the **learner/Q-matrix model**.

The next pass should revise the earlier five-scale Q-matrix using the
verified taxonomy and define a first practical latent-component set:
...
Then define how event-localized observations contribute qualitative
evidence to those components.

That will connect the authoritative fingering/domain work back to the
adaptive learner model and scheduler.

## 23. Document Status

This revision converts the initial proposed motor taxonomy into a
**mechanically verified V1 taxonomy**.

The two-family hypothesis from the first draft has been rejected because
the supposed `THREE_THEN_FOUR` and `FOUR_THEN_THREE` cycles are
rotations of the same cyclic structure.

For the current canonical 48-scale / 96-hand V1 corpus, one
`DIATONIC_3_4_CYCLE` motor family is sufficient. Future data may expand
the taxonomy, but no additional canonical V1 family is justified by the
current corpus.
