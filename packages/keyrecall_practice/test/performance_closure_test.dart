...
void main() {
  ...
  test(
    'an underchallenged attempt defers its faster probe for one slot',
    () async {
      final store = InMemoryPracticeStore(createdAt: t0);
      final session = await openSession(store);
      final presented = await presentUntil(
      ...
      final probe = session.session.tempoProbe!;
      final intervening = await session.decide(at: t0.plusDays(1));
      expect(intervening, isNotNull);
      expect(intervening!.exercise, isNot(probe));
      expect(
        intervening.decision.decision.challengeBypass,
        isNot(ChallengeBypass.tempoProbe),
      );
      await session.closeWithOutcome(outcomeFor(intervening.exercise));
      expect(session.session.tempoProbe, probe);
      expect(session.session.tempoProbeIsFresh, isFalse);

      final next = await session.decide(at: t0.plusDays(2));
      ...
      expect(
        next.decision.decision.challengeBypass,
